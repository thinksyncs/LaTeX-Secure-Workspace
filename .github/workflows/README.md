...
Stable Marketplace publication is therefore automatic only after the required push CI, including Security Guardrails, is green on the current `master` commit and no GitHub release already exists for the `package.json` version.
