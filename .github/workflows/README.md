...
The Docker build workflow separately verifies real PDF output from both fixed secure profiles, a rejected write through the read-only source mount, an output-local LuaTeX cache, disabled shell escape, and disabled Lua sockets. It is one of the workflows required by `auto-stable-release.yml`.
