...
const LOG_PANEL = vscode.window.createOutputChannel('LaTeX Workspace Security', 'latex_workshop_log')
...
function showErrorMessageWithExtensionLogButton(message: string) {
    const res = showErrorMessage(message, 'Open LaTeX Workspace Security log')
    if (res) {
        return res.then(option => {
            switch (option) {
                case 'Open LaTeX Workspace Security log': {
                    showLog()
                    break
                }
                default: {
