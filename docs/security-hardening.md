...
The secure build disables auto build, ignores custom recipes and tools, and ignores external build commands. Manual compilation is limited to fixed internal pdfLaTeX and LuaLaTeX recipes. Both pass `-no-shell-escape` to `latexmk`. By default both profiles use the hardened Docker wrapper, which disables networking, mounts the owning workspace read-only, exposes only `.lw-security` as a writable output mount, drops capabilities, and prevents privilege gain. The LuaLaTeX recipe also disables the Lua socket library. Host pdfLaTeX remains available only through the `latex-workshop.security.allowLocalPdfLaTeX` user setting as a weaker compatibility mode; workspace settings cannot enable it, and it is not a filesystem sandbox.
