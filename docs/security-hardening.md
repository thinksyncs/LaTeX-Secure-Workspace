...
The secure build disables auto build, ignores custom recipes and tools, and ignores external build commands. Manual compilation is limited to fixed internal pdfLaTeX and LuaLaTeX recipes. Both pass `-no-shell-escape` to `latexmk`. Docker builds disable networking, mount the owning workspace read-only, expose only `.lw-security` as a writable output mount, drop capabilities, and prevent privilege gain. LuaLaTeX requires Docker and disables the Lua socket library. Host pdfLaTeX requires explicit consent through `latex-workshop.security.allowLocalPdfLaTeX` in User Settings; workspace settings cannot enable it, and it is not a filesystem or network sandbox.

Optional TinyTeX setup is a separate network and executable-installation operation, not part of the Docker boundary. Only explicit profile selection and download confirmation in a trusted local workspace start it. The extension pins release assets, sizes, and SHA-256 values and permits HTTPS downloads only from GitHub's release hosts and `ctan.net` (Japanese font packages). It extracts into private staging before checking the tools. On Windows the verified bundle is a self-extracting executable. The Japanese profile adds CJK and IPAex Mincho/Gothic, including licenses, only to its own TeX tree. A matching hash establishes the pinned bytes, not that upstream code is harmless. Installation does not request elevation, change OS PATH, register OS fonts, upload documents, or automatically update packages. Other installations and incomplete destinations are preserved. Remote extension hosts are not modified. `security.useManagedTeX` and `security.managedTeXProfile` use User Settings only; project settings cannot select a toolchain.

On Windows, build and diagnostic launchers are resolved to installed paths outside the workspace before entering the document directory. Empty, relative, and project-local PATH entries are excluded. The local recipe also binds pdfLaTeX, bibliography, indexing, and lookup tools through an extension-owned latexmk policy; unavailable optional tools fail when needed. Batch-wrapper paths and arguments containing percent signs or line breaks are rejected to prevent shell reinterpretation. This prevents project-directory executable substitution on these paths, but does not attest the installed binaries or sandbox their file access. A TeX installation inside the workspace must be moved outside it to use these build paths.
