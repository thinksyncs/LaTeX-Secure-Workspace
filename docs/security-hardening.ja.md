...
Windows のビルドと診断では、文書ディレクトリへ移る前に、ワークスペース外にある導入済みツールの実行パスを解決します。PATH の空要素、相対パス、プロジェクト内のディレクトリは除外します。ローカルビルドでは、拡張機能が持つ latexmk ポリシーで pdfLaTeX・文献・索引・ファイル検索ツールも固定し、任意ツールがなければ必要になった時点で停止します。これは対象経路でプロジェクト内の偽実行ファイルが選ばれることを防ぎますが、導入済みバイナリの安全性の証明やファイルアクセスの隔離ではありません。ワークスペース内に置いた TeX は、これらのビルド経路で使うには外へ移す必要があります。
