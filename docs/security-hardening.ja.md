...
このセキュアビルドでは、自動ビルドを無効化し、カスタムのビルドレシピとツールを無視し、外部ビルドコマンドも使いません。手動ビルドは固定の pdfLaTeX / LuaLaTeX 内部レシピだけに限定し、両方で `-no-shell-escape` を使います。Docker ビルドではネットワークを無効化し、所属ワークスペースを読み取り専用でマウントし、書き込み先を `.lw-security` に限定します。capability を破棄し、権限昇格も禁止します。LuaLaTeX には Docker が必要で、Lua のソケット機能も無効化します。ホスト上の pdfLaTeX には `latex-workshop.security.allowLocalPdfLaTeX` のユーザー設定による明示的な同意が必要です。ワークスペース設定からは有効化できず、ファイルシステムやネットワークの隔離は提供しません。

任意の TinyTeX 導入は、Docker の隔離とは別の通信・実行ファイル導入処理です。信頼済みのローカルワークスペースで構成を選び、ダウンロードを明示承認した場合だけ開始します。配布物の版・容量・SHA-256 を固定し、HTTPS の接続先を GitHub のリリース用ホストと日本語パック用の `ctan.net` に限定します。専用の一時領域へ展開してからツールを確認し、Windows では検証済みの自己展開形式を実行します。日本語版は CJK と IPAex 明朝・ゴシック、ライセンスを専用の TeX 内に追加します。ハッシュ一致は固定したバイト列との一致であり、配布元のコードが無害である証明ではありません。管理者権限の要求、OS の PATH 変更、OS へのフォント登録、文書アップロード、パッケージの自動更新は行いません。既存の TeX と不完全な導入先を保持し、リモートホストも変更しません。`security.useManagedTeX` と `security.managedTeXProfile` はユーザー設定だけを読み、プロジェクト設定から実行ツールを選択できません。

Windows のビルドと診断では、文書ディレクトリへ移る前に、ワークスペース外にある導入済みツールの実行パスを解決します。PATH の空要素、相対パス、プロジェクト内のディレクトリは除外します。ローカルビルドでは、拡張機能が持つ latexmk ポリシーで pdfLaTeX・文献・索引・ファイル検索ツールも固定し、任意ツールがなければ必要になった時点で停止します。バッチ経由のコマンドのパスや引数にパーセント記号・改行がある場合は、シェルによる再解釈を防ぐため拒否します。これは対象経路でプロジェクト内の偽実行ファイルが選ばれることを防ぎますが、導入済みバイナリの安全性の証明やファイルアクセスの隔離ではありません。ワークスペース内に置いた TeX は、これらのビルド経路で使うには外へ移す必要があります。

安全なルート解決では、実体が現在のワークスペース外にある subfiles ルートを拒否します。ビルドとクリーンアップで使う `.lw-security` とその配下の作成先についても、シンボリックリンクではなく、プロジェクト内の実ディレクトリであることを確認します。
