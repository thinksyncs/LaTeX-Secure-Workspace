...
このセキュアビルドでは、自動ビルドを無効化し、カスタムのビルドレシピとツールを無視し、外部ビルドコマンドも使わないようにしています。手動ビルドは固定の内部レシピだけに限定し、ワークスペースの内容や設定からビルドコマンド経路を切り替えられないようにしています。固定レシピは `latexmk` に `-no-shell-escape` を渡し、TeX の `\write18` によるコマンド実行も無効化します。
