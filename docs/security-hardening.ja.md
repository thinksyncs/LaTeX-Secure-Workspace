...
このセキュアビルドでは、自動ビルドを無効化し、カスタムのビルドレシピとツールを無視し、外部ビルドコマンドも使いません。手動ビルドは固定の pdfLaTeX / LuaLaTeX 内部レシピだけに限定し、どちらも既定では強化済み Docker ラッパーを使います。ラッパーはネットワークを無効化し、所属ワークスペースを読み取り専用でマウントし、書き込み先を `.lw-security` の出力マウントに限定します。capability を破棄し、権限昇格も禁止します。両レシピは `latexmk` に `-no-shell-escape` を渡し、LuaLaTeX では Lua のソケット機能も無効化します。ホスト上の pdfLaTeX は、`latex-workshop.security.allowLocalPdfLaTeX` をユーザー設定で明示した場合だけ使える互換モードです。ワークスペース設定からは有効化できず、ファイルシステム隔離も提供しません。
