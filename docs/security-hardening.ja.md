...
このセキュアビルドでは、自動ビルドを無効化し、カスタムのビルドレシピとツールを無視し、外部ビルドコマンドも使わないようにしています。手動ビルドは固定の pdfLaTeX / LuaLaTeX 内部レシピだけに限定し、ワークスペースの内容や設定からビルドコマンド経路を切り替えられないようにしています。どちらの固定レシピも `latexmk` に `-no-shell-escape` を渡し、TeX の `\write18` によるコマンド実行も無効化します。LuaLaTeX は文書内の Lua を実行できます。また、`--safer` は `luaotfload` と互換性がないため、固定の LuaLaTeX プロファイルは Docker 経由でのみ実行します。Docker ラッパーはネットワークを無効にし、ソースを読み取り専用でマウントし、ホスト側の書き込み先を出力用マウントに限定します。レシピ側でも Lua のソケット機能を無効にします。
