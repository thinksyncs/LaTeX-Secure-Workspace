...
`node dev/testManagedTex.mjs lightweight` and `node dev/testManagedTex.mjs japanese` (after compile) explicitly download pinned bundles into disposable storage. They build with production recipe arguments, check reuse without another download, and inspect PDF text and embedded Japanese fonts. Both profiles run on Linux, macOS, and Windows with system-only PATH, spaces in paths, and saved PDF/JSON evidence. No normal VS Code profile or OS PATH is changed. This matrix gates automatic stable releases. Unit tests mock network/install operations for profile choice, consent, cancellation, checksums, redirect boundaries, and existing-directory preservation.

The Docker test does not pull an implicit image. For a local run, first pull the digest-pinned image in the [Docker setup](../docs/manual/README.md#optional-docker-setup) and set `LATEXWORKSHOP_DOCKER_TEST_IMAGE` to the same `repository@sha256:...` value. This keeps the test input explicit and reproducible.
